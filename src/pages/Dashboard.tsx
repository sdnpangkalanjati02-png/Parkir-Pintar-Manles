...
export default function Dashboard() {
  ...
  const kpis = [
    { title: 'Kendaraan Terparkir', value: stats.occupied, icon: Users, color: 'text-blue-600', bg: 'bg-blue-50', trend: '+12%', isUp: true },
    ...(isManagement ? [{ title: 'Total Pendapatan', value: `Rp ${stats.revenueToday.toLocaleString()}`, icon: DollarSign, color: 'text-emerald-600', bg: 'bg-emerald-50', trend: '+8.4%', isUp: true }] : []),
    { title: 'Total Kendaraan Masuk', value: stats.entriesToday, icon: TrendingUp, color: 'text-purple-600', bg: 'bg-purple-50', trend: '-2.1%', isUp: false },
    { title: 'Tingkat Okupansi', value: `${((stats.occupied / stats.totalCapacity) * 100).toFixed(1)}%`, icon: Clock, color: 'text-orange-600', bg: 'bg-orange-50', trend: '+4.5%', isUp: true },
  ];
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                {isSyncing ? 'Sinkronisasi...' : `Live: Diperbarui ${format(lastRefreshed, 'HH:mm:ss')}`}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className={cn("space-y-8", isManagement ? "lg:col-span-2" : "lg:col-span-3")}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider">Aliran Pendapatan</h3>
                ...
                <div className="h-48 px-2">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={processedChartData}>
                      ...
                      />
                      <Area type="monotone" dataKey="revenue" stroke="#3B82F6" strokeWidth={3} fillOpacity={1} fill="url(#colorRevenue)" name="Pendapatan (Rp)" />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider">Distribusi Kendaraan</h3>
                  <div className="text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-1 rounded">PER 2 JAM</div>
                </div>
                <div className="h-48 px-2">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={processedChartData}>
                      ...
                      <Bar dataKey="motorcycles" fill="#94A3B8" radius={[4, 4, 0, 0]} name="Motor" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                    ...
                    <tr key={log.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <span className={cn(
                          ...
                          {log.status === 'parked' ? 'Di Dalam' : 'Keluar'}
                        </span>
                      </td>
                  ...
                  {filteredLogs.length === 0 && (
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center text-slate-400 font-medium">
                        Tidak ada aktivitas terdeteksi untuk jenis kendaraan ini.
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100 text-center">
              <button className="text-blue-600 text-sm font-semibold hover:underline">Lihat Semua Aktivitas</button>
            </div>
          </div>
        </div>
        ...
        <div className={cn("space-y-8", !isManagement && "lg:col-span-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-8 space-y-0 lg:space-y-8")}>
          <div className="bg-blue-600 rounded-2xl p-6 text-white shadow-lg shadow-blue-200">
            <h4 className="text-lg font-bold mb-4">Status Kapasitas</h4>
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-sm mb-1 opacity-80">
                  <span>Penggunaan</span>
                  <span>{stats.occupied} / {stats.totalCapacity} Slot</span>
                </div>
              ...
              <p className="text-xs opacity-70 leading-relaxed">
                Deteksi lot cerdas aktif. Perutean optimal untuk kendaraan masuk diaktifkan.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
            ...
            <h4 className="font-bold text-slate-800 mb-4 text-sm uppercase tracking-wider">Akses Cepat</h4>
